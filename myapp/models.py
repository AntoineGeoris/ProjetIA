...
class History(db.Model) :
	no_turn = db.Column(db.Integer, db.ForeignKey('gameBoard.no_turn'))
	player_1_pos = db.Column(db.String(2), db.ForeignKey('gameBoard.player_1_pos'), nullable = False)
	player_2_pos = db.Column(db.String(2), db.ForeignKey('gameBoard.player_1_pos'), nullable = False)
	board = db.Column(db.String(25),db.ForeignKey('gameBoard.board'), nullable = False)
	move =  db.Column(db.String(5))
	id = db.Column(db.Integer, db.ForeignKey('gameBoard.id'), primary_key = True) # ????

class QTableState(db.Model):
	#id = db.Column(db.Integer, primary_key = True) # Pas besoin ? Voir state ?
	state = db.Column(db.String(32)) #25 board + 3 digits turn no + 4 digits for player position ? Primary key ? 
